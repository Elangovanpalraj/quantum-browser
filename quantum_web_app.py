...
def ask():
    ...
    history_rows = db.execute(
        "SELECT role, content FROM chat_history WHERE user_id=? ORDER BY id DESC LIMIT 6",
        (uid,)
    ).fetchall()
    history = [{"role": r["role"], "content": r["content"][:1500]} for r in reversed(history_rows)]

    # Groq free tier only allows ~8000 tokens/minute total. A very long pasted
    # message (like a big code dump) can blow past that on its own, so cap it.
    MAX_MESSAGE_CHARS = 6000
    if len(message) > MAX_MESSAGE_CHARS:
        message = message[:MAX_MESSAGE_CHARS] + "\n\n[...trimmed, message was too long...]"
